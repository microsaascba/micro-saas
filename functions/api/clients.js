...
export async function onRequestPost(context) {
  try {
    ...
    `).bind(data.id, data.name, data.contact, data.phone, data.email, data.cuil, data.address, data.fee, data.dueDate, 1, data.adminUser, data.adminPass, data.type || 'client', new Date().toISOString()).run();
...
export async function onRequestPut(context) {
  try {
    ...
    `).bind(data.name, data.contact, data.phone, data.email, data.cuil, data.address, data.fee, data.dueDate, data.active ? 1 : 0, data.adminUser, data.adminPass, data.type || 'client', data.id).run();
...
export async function onRequestDelete(context) {
  try {
    ...
    // SI LE MANDAMOS LA ORDEN DE REVOCAR, BORRA AL USUARIO DEL SISTEMA PERO DEJA EL DATO EN EL MASTER
    if (revoke === 'true') {
    ...
    // SI NO, BORRA EL CLIENTE COMPLETO
